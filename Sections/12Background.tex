...
\Zeb{Main arguments: phonemes used to have more of a practical application in speech recognition technology, but end-to-end with raw audio has replaced these, as discussed in the last section. The use of phonemes for  have had some practical applications, they are primarily used for linguistic research that presupposes phonemes as the base unit. We focus on word segmentation as one case study, finding that there are close ties with modern subword tokenisation algorithms. In \cref{sec:12-phonexperiment} we present other examples of the phonemic representation being used in phonological experimentation, noting that there are few examples of LMs being trained on running text for this purpose. It is of interest }

Phoneme-based input representations consist of tokens representing individual phonemes. This has similarities to character-based representations and in many languages there is a close correspondence between characters and phonemes, but many languages have much more opaque orthographies, like English. Additionally, in contrast to character-based representations, phoneme-based representations typically do not include punctuation, whitespace or other artefacts of written text. In particular, this means that words are not delimited, since spoken language typically does not contain such clear word boundaries \addcites. 

As discussed in the previous section, constructing phoneme language models used to be foundational to speech processing technology, but recent architectural advances now operate directly from audio, making these models largely obsolete for practical speech processing systems. A central task for training these language models was \defn{grapheme-to-phoneme conversion} \citep[G2P][]{lucassen1984information}, allowing orthographic transcriptions to be converted into to phonemes. Despite phoneme language models becoming obsolete for large-scale speech processing technology, there is still interest in the G2P task. This is because models that train with speech-based representations require substantially more data than prior frameworks, which makes low-resource languages difficult to model.

\citep[e.g.][]{bisani-2008-g2p,hasegawa-2020-g2pmultilingual}.
%Cross-linguality, alignment with speech

Additionally, phonemes are a far more universal representation than orthographic text, with scripts that varies considerably across languages and cultures. Phonemic representation has been argued to be effective for low-resource multilingual modelling, but a lack of data... There are also a few specific tasks where phoneme-based representations seem useful, like lyric generation and ...

Where phoneme representations have seen continual use is in linguistic research that presupposes phonemes as the base abstraction. We focus on word segmentation as one case study in \cref{sec:12-wordseg}, finding that there are close ties with modern subword tokenisation algorithms. In \cref{sec:12-phonexperiment} we present other examples of the phonemic representation being used in phonological experimentation, from early connectionist networks to modern LMs, where the later could still provide unique insights about the distributional properties of language.

Finally, phoneme-based representations are useful for gaining insights into language model training as a diagnostic valuation. For instance, what advantage does orthographic text give language models --- phoneme-based representations could act as a confounding factor to study this. Note that there are relatively few examples of using the phoneme-based representation to train LMs on full utterances, most G2P work operates on words, similarly to the phonological experimentation, and word segmentation models primarily exist pre-LM. \cref{sec:12-phoneval} discusses the limited work that does train phoneme LMs and how they have been evaluated. 

% CHAT: Introduce the tension: LMs are often tested on phonological knowledge despite being trained on orthographic input.
...
\subsection{Implementation of phoneme-based input}\cref{sec:12-implementphoneme}
...
\subsection{Evaluation of phoneme language models}\label{sec:12-phoneval}

One benchmark that assesses both the syntactic and phonological capabilities of language models is BabySLM \citep{lavechin}. %One distinguishing advantage of this benchmark is that it can be applied to models trained on either graphemes or phonemes.
We discuss this benchmark further in \cref{sec:14-effect}.


It is also a challenge to evaluate the broad capabilities of language models trained with phonemes, as most benchmarks assume a graphemic representation, even some that assess phonological knowledge \citep{suvarna-etal-2024-phonologybench}. 


Several language models have been trained with phonemic input \citep{sundararaman-2021-phonemebert, gale-etal-2023-bort} but it remains a challenge to do so due to the lack of large phonemic corpora. While a number of well-known speech-based datasets include phonemic transcriptions, such as Switchboard \citep{godfrey1992switchboard} and TIMIT \citep{garofolo1993darpa}, these datasets are small compared to the trillions of tokens contained in standard language model pre-training corpora \citep{elazar-2024-redpajama}. 
