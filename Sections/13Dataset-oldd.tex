...
%\rough{Clearly none match all the requirements we want for the work in this thesis, a good solution is \textbf{automatic phonemization} to convert CHILDES and BabyLM Pre-training data. Talk about how some even fully synthesize speech and then use ASR to get phonemic data. E.g.  with switchboard}
...
% - Possibly add vox lingua (youtube 107 languages, https://arxiv.org/abs/2011.12998, audio-only) and voxpopuli (parliament)
% - Definitely need to add vox Communis https://aclanthology.org/2022.lrec-1.566/ (acoustic + phonemic derived from common voice)
% - also VoxClamantis https://arxiv.org/abs/2005.13962 (new testament readings)

%In general, there are trade-offs between the properties of the datasets. The main challenge seems to be finding \textbf{phonemic} datasets that are sufficiently \textbf{large} for training transformer-based language models. It is then even more challenging to find datasets that also match the particular domain and language requirements of this thesis. If CHILDES were available in a phonemic format, it would be a suitable dataset for this work. One solution to this problem is to use \textbf{automatic phonemization} to convert orthographic datasets to a phonemic format. This is the approach I take in this thesis.

% \subsection{Phoneme streams}
% \label{sec:dataset-phoneme-streams}

% - paper that uses phoible:
% https://ieeexplore.ieee.org/document/9054362
...
%In this section, I compare each language in \ipachildes according to their information-theoretic properties,   
